...
class PartnersOpenInvoicesWebkit(report_sxw.rml_parse,
    ...
    def __init__(self, cursor, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'get_subsidiary': self._get_subsidiary_br,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        ledger_lines_memoizer = self._compute_open_transactions_lines(
            account_ids, subsidiary_ids, main_filter, target_move, start, stop, date_until,
        ...
        for subsidiary_id in subsidiary_ids:
            ledger_lines[subsidiary_id] = {}
            init_balance[subsidiary_id] = {}
            partners_order[subsidiary_id] = {}
            init_balance_memoizer[subsidiary_id] = {}
            for account in objects:
                ledger_lines[subsidiary_id][account.id] = ledger_lines_memoizer[subsidiary_id].get(account.id,
                                                                     {})
                init_balance[subsidiary_id][account.id] = init_balance_memoizer[subsidiary_id].get(account.id,
                                                                     {})
                ...
                ledg_lines_pids = ledger_lines_memoizer[subsidiary_id].get(account.id, {}).keys()
                non_null_init_balances = dict([
                    (ib, amounts) for ib, amounts
                    in init_balance[subsidiary_id][account.id].iteritems()
                    if amounts['init_balance']
                    or amounts['init_balance_currency']])
                init_bal_lines_pids = non_null_init_balances.keys()
                
                partners_order[subsidiary_id][account.id] = self._order_partners(
    ...
    def _compute_open_transactions_lines(self, accounts_ids, subsidiary_ids, main_filter,
        ...
        for subsidiary_id in subsidiary_ids:
            ...
            for account_id in accounts_ids:
                ...
                move_line_ids_per_partner = self.get_partners_move_lines_ids(
                    account_id, subsidiary_id, main_filter, start, stop, target_move,

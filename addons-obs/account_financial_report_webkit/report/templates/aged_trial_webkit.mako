...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        %for subsidiary_id in get_subsidiary(data):
	        %for acc in objects:
	          %if agged_lines_accounts[subsidiary_id.id][acc.id]:
	          	
	          	<div class="account_title bg" style="width: 1080px; margin-top: 20px; font-size: 12px;">${subsidiary_id.name}</div>
		        <div class="account_title bg" style="width: 1080px; margin-top: 20px; font-size: 12px;">${acc.code} - ${acc.name}</div>
	                ...
	                <div class="act_as_table list_table" style="margin-top: 5px;">
	                  ...
	                  <div class="act_as_tbody">
	                    %for partner_name, p_id, p_ref, p_name in partners_order[subsidiary_id.id][acc.id]:
	                       %if agged_lines_accounts[subsidiary_id.id][acc.id].get(p_id):
	                       <div class="act_as_row lines">
	                         <%line = agged_lines_accounts[subsidiary_id.id][acc.id][p_id]%>
	                         <%percents = agged_percents_accounts[subsidiary_id.id][acc.id]%>
	                         <%totals = agged_totals_accounts[subsidiary_id.id][acc.id]%>

...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
		...
		%for subsidiary_id in get_subsidiary(data):
			%if ledger_lines[subsidiary_id.id] and ledger_lines[subsidiary_id.id] or init_balance[subsidiary_id.id] and init_balance[subsidiary_id.id]:
				<div class="account_title bg" style="width: 1080px; margin-top: 20px; font-size: 12px; text-decoration:underline;">${subsidiary_id.name}</div>
		        %for account in objects:
		        	
		            %if ledger_lines[subsidiary_id.id] and ledger_lines[subsidiary_id.id][account.id] or init_balance[subsidiary_id.id] and init_balance[subsidiary_id.id][account.id]:
		                <%
		                if not partners_order[subsidiary_id.id][account.id]:
		                    continue
		                ...
		                %for partner_name, p_id, p_ref, p_name in partners_order[subsidiary_id.id][account.id]:
		                <%
		                  total_debit = 0.0
		                  total_credit = 0.0
		                  cumul_balance = 0.0
		                ...
		                <div class="act_as_table list_table" style="margin-top: 5px;">
		                    ...
		                    <div class="act_as_tbody">
		                        <%
		                        total_debit = init_balance[subsidiary_id.id][account.id].get(p_id, {}).get('debit') or 0.0
		                        total_credit =init_balance[subsidiary_id.id][account.id].get(p_id, {}).get('credit') or 0.0
		                        %>
		                          %if initial_balance_mode and (total_debit or total_credit):
		                            <%
		                              part_cumul_balance = init_balance[subsidiary_id.id][account.id].get(p_id, {}).get('init_balance') or 0.0
		                              part_cumul_balance_curr = init_balance[subsidiary_id.id][account.id].get(p_id, {}).get('init_balance_currency') or 0.0
		                              balance_forward_currency = init_balance[subsidiary_id.id][account.id].get(p_id, {}).get('currency_name') or ''
		                        ...
		                        %for line in ledger_lines[subsidiary_id.id][account.id].get(p_id, []):
		                          <%

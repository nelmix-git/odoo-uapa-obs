...
class AccountAgedTrialBalanceWebkit(PartnersOpenInvoicesWebkit):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        for subsidiary_id in subsidiary_ids:
            agged_lines_accounts[subsidiary_id] = {}
            agged_totals_accounts[subsidiary_id] = {}
            agged_percents_accounts[subsidiary_id] = {}
            for acc in self.objects:
                agged_lines_accounts[subsidiary_id][acc.id] = {}
                agged_totals_accounts[subsidiary_id][acc.id] = {}
                agged_percents_accounts[subsidiary_id][acc.id] = {}
                
                for part_id, partner_lines in\
                        self.localcontext['ledger_lines'][subsidiary_id][acc.id].items():
    
                    aged_lines = self.compute_aged_lines(part_id,
                                                         partner_lines,
                                                         data)
                    if aged_lines:
                        agged_lines_accounts[subsidiary_id][acc.id][part_id] = aged_lines
                agged_totals_accounts[subsidiary_id][acc.id] = totals = self.compute_totals(
                    agged_lines_accounts[subsidiary_id][acc.id].values())
                agged_percents_accounts[subsidiary_id][acc.id] = self.compute_percents(totals)

...
class CommonPartnersReportHeaderWebkit(CommonReportHeaderWebkit):
    ...
    def get_partners_move_lines_ids(self, account_id, subsidiary_id, main_filter, start, stop,
                                    target_move,
        ...
        if filter_from:
            return self._get_partners_move_line_ids(
                filter_from, account_id, subsidiary_id, start, stop, target_move,
    ...
    def _get_partners_move_line_ids(self, filter_from, account_id, subsidiary_id, start, stop,
                                    target_move,
        ...
        sql_where = " WHERE account_move_line.account_id = %(account_ids)s " \
                    " AND account_move_line.subsidiary_id = %(subsidiary_ids)s " \
    ...
    def _partners_initial_balance_line_ids(self, account_ids, subsidiary_ids, start_period,
                                           partner_filter,
                                           exclude_reconcile=False,
        ...
        search_param = {
            ...
            'subsidiary_ids': tuple(subsidiary_ids),
        }
        sql = ("SELECT ml.id, ml.account_id, ml.partner_id, ml.subsidiary_id "
               "FROM account_move_line ml "
               "INNER JOIN account_account a "
               "ON a.id = ml.account_id "
               "WHERE ml.period_id in %(period_ids)s "
               "AND ml.account_id in %(account_ids)s "
               "AND ml.subsidiary_id in %(subsidiary_ids)s ")
        if exclude_reconcile:
            if not date_stop:
                raise Exception(
    ...
    def _compute_partners_initial_balances(self, account_ids, subsidiary_ids, start_period,
                                           partner_filter=None,
                                           exclude_reconcile=False,
                                           force_period_ids=False):
        ...
        if isinstance(subsidiary_ids, (int, long)):
            subsidiary_ids = [subsidiary_ids]
            
        move_line_ids = self._partners_initial_balance_line_ids(
            account_ids, subsidiary_ids, start_period, partner_filter,
            exclude_reconcile=exclude_reconcile,
            force_period_ids=force_period_ids)
        if not move_line_ids:
            move_line_ids = [{'id': -1}]
        sql = ("SELECT ml.account_id, ml.partner_id, ml.subsidiary_id,"
               ...
               "LEFT JOIN res_company_subsidiary sub "
               "ON sub.id = ml.subsidiary_id "
               "WHERE ml.id in %(move_line_ids)s "
               "GROUP BY ml.account_id, ml.partner_id, ml.subsidiary_id, a.currency_id, c.name")
        search_param = {

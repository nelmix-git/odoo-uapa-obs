...
class CommonPartnerBalanceReportHeaderWebkit(CommonBalanceReportHeaderWebkit,
    ...
    def _get_account_partners_details(self, account_by_ids, subsidiary_ids, main_filter,
        ...
        for subsidiary_id in subsidiary_ids:
            final_res[subsidiary_id] = {}
            partners_init_balances_by_ids[subsidiary_id] = {}
            res = {}
            for account_id, account_details in account_by_ids.iteritems():
                partners_init_balances_by_ids[subsidiary_id].update(
                    self._get_partners_initial_balances(
                        account_id, subsidiary_id, start, initial_balance_mode,
            ...
            final_res[subsidiary_id] = res

        return res

    def _get_partners_initial_balances(self, account_ids, subsidiary_ids, start_period,
                                       initial_balance_mode,
        ...
        if initial_balance_mode == 'opening_balance':
            ...
            res = self._compute_partners_initial_balances(
                account_ids, subsidiary_ids, start_period, partner_filter_ids,
    ...
    def _get_partners_totals_account(self, filter_from, account_id, subsidiary_id, start,
                                     stop, target_move,
                                     partner_filter_ids=None,
        ...
        sql_where = "WHERE account_move_line.account_id = %(account_id)s \
                    AND account_move_line.subsidiary_id = %(subsidiary_id)s \
        ...
        search_params.update({'account_id': account_id, 'subsidiary_id': subsidiary_id})
    ...
    def _get_partners_comparison_details(self, data, account_ids, subsidiary_id, target_move,
        ...
        if comparison_filter != 'filter_no':
            ...
            accounts_by_ids = self._get_account_details(
                account_ids, [subsidiary_id], target_move, fiscalyear, details_filter, start,
                stop, initial_balance_mode)

            partner_details_by_ids = self._get_account_partners_details(
                accounts_by_ids, [subsidiary_id], details_filter,
    ...
    def compute_partner_balance_data(self, data, filter_report_type=None):
        subsidiary_obj = self.pool.get('res.company.subsidiary')
        ...
        subsidiary_ids = self._get_form_param('subsidiary_ids', data)
        ...
        if not subsidiary_ids:
            subsidiary_ids = subsidiary_obj.search(self.cr, self.uid, [])
        ...
        for subsidiary_id in subsidiary_ids:
            subsidiary_dict[subsidiary_id] = {}
            accounts_by_ids = self._get_account_details(
                account_ids, [subsidiary_id], target_move, fiscalyear, main_filter, start, stop,
                initial_balance_mode)
            subsidiary_dict[subsidiary_id] = accounts_by_ids
        
            partner_details_by_ids = self._get_account_partners_details(
                accounts_by_ids, [subsidiary_id], main_filter, target_move, start, stop,
            ...
            init_balance_accounts[subsidiary_id] = {}
            comparisons_accounts[subsidiary_id] = {}
            partners_order_accounts[subsidiary_id] = {}
            partners_amounts_accounts[subsidiary_id] = {}
            debit_accounts[subsidiary_id] = {}
            credit_accounts[subsidiary_id] = {}
            balance_accounts[subsidiary_id] = {}
    
            for account in objects:
                if not account.parent_id:  # hide top level account
                    continue
                debit_accounts[subsidiary_id][account.id] = accounts_by_ids[account.id]['debit']
                credit_accounts[subsidiary_id][account.id] = accounts_by_ids[account.id]['credit']
                balance_accounts[subsidiary_id][account.id] = \
                    accounts_by_ids[account.id]['balance']
                init_balance_accounts[subsidiary_id][account.id] = accounts_by_ids[
                    account.id].get('init_balance', 0.0)
                partners_amounts_accounts[subsidiary_id][account.id] =\
                ...
                for comp_account_by_id in comp_accounts_by_ids:
                    ...
                    for partner_id, partner_values in \
                        ...
                        base_partner_balance = partners_amounts_accounts[subsidiary_id][account.id][partner_id]['balance']\
                            if partners_amounts_accounts[subsidiary_id].get(account.id)\
                            and partners_amounts_accounts[subsidiary_id].get(account.id)\
                ...
                comparisons_accounts[subsidiary_id][account.id] = comp_accounts
                ...
                partners_order_accounts[subsidiary_id][account.id] = \

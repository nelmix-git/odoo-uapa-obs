...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def _get_subsidiary_br(self, data):
        subsidiary_obj = self.pool.get('res.company.subsidiary')
        if data.get('form', {}).get('subsidiary_ids'):
            return self._get_info(data, 'subsidiary_ids', 'res.company.subsidiary')
        else:
            subsidiary_ids = subsidiary_obj.search(self.cr, self.uid, [])
            return self.pool.get('res.company.subsidiary').browse(self.cursor, self.uid, subsidiary_ids)
...
FROM account_move_line l
    ...
    LEFT JOIN res_company_subsidiary sub on (l.subsidiary_id=sub.id)

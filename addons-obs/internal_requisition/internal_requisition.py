...
class HrDepartment(models.Model):
    ...
    analytic_account_id = fields.Many2one(comodel_name="account.analytic.account",
                                          string="Cuenta Analitica", required=False, )


class StockWarehouse(models.Model):
    _inherit = 'stock.warehouse'

    subsidiary_id = fields.Many2one(comodel_name="res.company.subsidiary", string="Subsidiaria", required=True, )
...
class internal_requisition(models.Model):
    ...
    def _get_default_warehouse(self):
        subsidiary_id = self.env['res.users'].subsidiary_id.id
        warehouse_ids = self.env['stock.warehouse'].search([('subsidiary_id', '=', subsidiary_id)], limit=1)
    ...
    department_id = fields.Many2one('hr.department', string='Department', readonly=False, required=True,
                                    states={'draf': [('readonly', True)]},
    ...
    journal_id = fields.Many2one('account.journal', 'Expense Journal', required=False, readonly=True,
                                 default=lambda self: self.env.user.subsidiary_id.ir_journal_id.id,
                                 domain="[('type','=','purchase')]", help='Journal related to stock Accounting Entries')
    state = fields.Selection([
                       ('draft', 'Nueva'),
                       ('confirm', 'Confirmada por el usuario / Esperando Validacion Gerente'),
                       ('cancel', 'Cancelled'),
                       ('valid', 'Validada por Gerente / Esperando Aprobacion Presupuestaria y Administrativa'),
                       ('approve', 'Approved - by Budget Department'),
                       ('pre_approve', 'Aprobada - by Direccion Administrativa'),
                       ('accounting', 'Para ser contabilizado por el Departamento de Contabilidad'),
                       ('waiting', 'Solicitud de Cotizacion Generada por el Departamento de Compra - Esperando Validacion y Recepcion de Almacen'),
                       ('delivery', 'Orden de Entrega Generada - Esperando Verificacion de Disponibilidad'),
                       ('ready', 'Listo para ser entregado por el Departamento de Almacen'),
                       ('done', 'Realizado'), ], string='State', default='draft', required=True, readonly=True)
    ...
    def confirm(self):
        ...
        return self.write({'state':'confirm', 'name': seq_no})
    ...
    def accounting_entry(self):
        ...
        for requisition in self:
            ...
            if requisition.budget_ids:
                ...
                analytic_account_id = requisition.department_id.analytic_account_id.id
            ...
            for line in requisition.line_ids:
                ...
                if line.product_id.valuation == 'real_time':
                    if line.product_id.categ_id.property_stock_account_output_categ.id:
                        debit_account_id = line.product_id.categ_id.property_stock_account_output_categ.id or False
                    else:
                        debit_account_id = line.product_id.product_tmpl_id.property_account_expense.id or False

                    if line.product_id.categ_id.property_stock_valuation_account_id.id:
                        credit_account_id = line.product_id.categ_id.property_stock_valuation_account_id.id or False
...
class internal_requisition_line(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Product', domain="[('type','!=','service'),('categ_id.available_for_ir','=',True)]", required=True)
